...
func String2ByteSliceConversion() {
	//// String - byte slice conversion
	str := "Yūgen ☯ 💀" // strings are immutable

	// convert string to byte slice
	sbytes := []byte(str) // creates a []byte and copies the bytes of the
	// string to the new slice's backing array
	sbytes[0] = 'N'
	sbytes[1] = 'Y'

	fmt.Printf("%s\n", str) // Yūgen ☯ 💀
	// changing the 'sbytes' doesn't change 'str' as they don't share the
	// same backing array

	// convert byte slice to string
	str = string(sbytes)
	// still they don't share same backing array, because
	// it creates a string and copies the bytes of the byte slice to the
	// new string's backing array
	fmt.Printf("%s\n", str) // NY?gen ☯ 💀
}

func CountBytesAndRunesInString() {
	//// Count bytes and runes in a string
	str := "Yūgen ☯ 💀"
	sbytes := []byte(str)

	fmt.Printf("%s\n", str)                                 // Yūgen ☯ 💀
	fmt.Printf("\t%d bytes\n", len(str))                    // 15 bytes, len(string) counts the bytes not the rune
	fmt.Printf("\t%d runes\n", utf8.RuneCountInString(str)) // 9 runes, counts rune in string
	// 9 runes but 15 bytes because some runes has multiple bytes

	fmt.Printf("% x\n", sbytes)                        // 4e 59 ab 67 65 6e 20 e2 98 af 20 f0 9f 92 80
	fmt.Printf("\t%d bytes\n", len(sbytes))            // 15 bytes, len([]bytes) counts the bytes not the rune
	fmt.Printf("\t%d runes\n", utf8.RuneCount(sbytes)) // counts rune in byte slice, 9 runes
	// 9 runes but 15 bytes because some runes has multiple bytes
}

func IndexAndSlicingString() {
	str := "Yūgen ☯ 💀"
	for i, r := range str {
		// fmt.Printf("str[%2d] = %-2x\n", i, str[i])
		fmt.Printf("str[%2d] = % -12x = %q\n", i, string(r), r)
...
Gezegenin maymun soyundan gelen canlıları öyle ilkeldir ki dijital kol saatinin hâlâ çok etkileyici bir buluş olduğunu düşünürler.`
	...
	// instead of using for loop, use the following function, its efficient
	_, size = utf8.DecodeRune(word)
